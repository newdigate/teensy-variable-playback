...
bool ResamplingLfsReader::isUsingSPI = false;

// read n samples into each buffer (1 buffer per channel)
unsigned int ResamplingLfsReader::read(void **buf, uint16_t nsamples) {
...
bool ResamplingLfsReader::readNextValue(int16_t *value, uint16_t channel) {
    ...
    newdigate::IndexableLittleFSFile<128, 2> &sourceBuffer = (*_sourceBuffer);
...
void ResamplingLfsReader::initializeInterpolationPoints(void) {
    if (_numChannels < 0)
        return;
...
void ResamplingLfsReader::deleteInterpolationPoints(void) {
...
void ResamplingLfsReader::begin(void)
{
...
bool ResamplingLfsReader::playRaw(const char *filename, uint16_t numChannels) {
    return play(filename, false, numChannels); 
}

bool ResamplingLfsReader::playWav(const char *filename) {
    return play(filename, true); 
}

bool ResamplingLfsReader::play(const char *filename, bool isWave, uint16_t numChannelsIfRaw)
{
    close();
    //system("echo ${PWD}");
    ...
    if (_file_size <= _header_offset * newdigate::IndexableLittleFSFile<128, 2>::element_size) {
        _playing = false;
        if (_filename) delete [] _filename;
        _filename =  nullptr;
    ...
    _sourceBuffer = new newdigate::IndexableLittleFSFile<128, 2>(_myFS, _filename);
...
void ResamplingLfsReader::reset(){
    initializeInterpolationPoints();
    if (_playbackRate > 0.0) {
...
int ResamplingLfsReader::available(void) {
    return _playing;
}

void ResamplingLfsReader::close(void) {
    if (_playing)
        stop();
